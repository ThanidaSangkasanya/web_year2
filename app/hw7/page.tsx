...
export default function Music() {
  ...
  const addInstrument = () => {
    if (editID !== -1) {
      ...
      const updatedInstruments = instruments.map((instrument, index) => 
        index === editID
          ? { ...instrument, name, price, image: imageUrl }
          : instrument
      );
      setinstruments(updatedInstruments);
      setEditID(-1); // for reset
    } else {
  ...
  function editTask(id: number) {
    const taskIndex = instruments.findIndex(item => item.id === id);
    setEditID(taskIndex); 
    setName(instruments[taskIndex].name);
    setPrice(instruments[taskIndex].price);
    setImageUrl(instruments[taskIndex].image);
  }
  
  function deleteTask(id: number) {
    const temp = instruments.filter((item) => item.id !== id)
    setinstruments([...temp])
  }
  ...
  return (
    <div className="bg-[#d5d0f7] min-h-screen">
      ...
      <div className='font-bold text-5xl p-5 text-center drop-shadow-xl'>Music Instruments</div>

      
      <div className="p-4 max-w-md mx-auto bg-white rounded-xl shadow-lg space-y-4 m-5">
        <h3 className="font-bold text-lg text-center">
        {editID == -1 ? "Add New Instrument" : "Edit"}</h3>
        <input
          type="text"
        ...
        <button
          ...
          className="bg-blue-500 text-white p-2 rounded mt-2 text-sm w-full hover:bg-blue-600"
        >
          {editID !== -1 ? "Confirm" : "Add Instrument"}
        </button>
...
<div className="flex flex-wrap gap-6 justify-center">
  {instruments.map((instrument, index) => (
    <Link key={instrument.id} href={`/params/${instrument.id}`} passHref>
      
      <div className="mt-5 group relative block overflow-hidden w-[300px] h-[500px]">
        ...
        <Image
          ...
          width={200}
          height={500}
          className="relative h-64 w-full object-cover transition duration-500 group-hover:scale-105 sm:h-72"
        />
        {instrument.is_new && (
          <span className="absolute top-2 left-2 whitespace-nowrap bg-red-400/80 px-3 py-1.5 text-xs font-medium text-white shadow-[2px_2px_10px_rgb(243,191,94)]">New</span>
        )}
        <div className="relative border bg-[#c297effd] p-3">
          <h3 className="font-bold mt-4 text-lg text-gray-900">{instrument.name}</h3>
          <p className="mt-1.5 text-sm text-gray-700">{instrument.price} ฿</p>
          <form className="mt-4 grid grid-cols-2 gap-4">
            ...
            <button
              type="button"
              className="block w-full rounded bg-[#5883ae] px-4 py-3 text-sm font-medium text-white transition hover:scale-105"
            >
              Buy Now
            </button>

            <button
              className="block w-full rounded bg-[#df3030] px-4 py-3 text-sm font-medium text-gray-900 transition hover:scale-105"
              onClick={(e) => {
                e.preventDefault(); 
                deleteTask(instrument.id);
              }}
              type="button"
            >
              Delete
            ...
            <button
              className="block w-full rounded bg-[#49d97e] px-4 py-3 text-sm font-medium text-gray-900 transition hover:scale-105"
              onClick={(e) => {
                e.preventDefault(); 
                editTask(instrument.id);
              }}
              type="button"
            >
              Edit
